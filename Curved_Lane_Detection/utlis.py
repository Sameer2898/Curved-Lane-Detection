...
def pipeline(img, s_thresh=(100, 255), sx_thresh=(15,255)):
    ...
    s_binary[(s_channel >= sx_thresh[0]) & (s_channel <= sx_thresh[1])] = 1
    color_binary = np.dstack((np.zeros_like(sxbinary), sxbinary, s_binary)) * 255
    combined_binary = np.zeros_like(sxbinary)
    combined_binary[(s_binary == 1) | (sxbinary == 1)] == 1
    return combined_binary

def perspective_warp(img, dst_size=(1280, 720), src=np.float32([(0.43, 0.65), (0.58, 0.65), (0.1, 1), (1, 1)]),
...
def textDisplay(curve, img):
    ...
    cv2.putText(img, directionText, ((img.shape[1]//2)-35, (img.shape[0])-20), font, 1, (0, 200, 200), 2, cv2.LINE__AA)
...
def drawLines(img, lane_curve):
    ...
    for x in range(-30, 30):
        w = myWidth // 20
        cv2.line(img, (w * x + int(lane_curve // 100), myHeight - 30), (w * x + int(lane_curve // 100), myHeight), (0, 0, 255), 2)
    cv2.line(img, (int(lane_curve // 100) + myWidth // 2, myHeight - 30), (int(lane_curve // 100) + myWidth //2, myHeight), (0, 255, 0), 3)
